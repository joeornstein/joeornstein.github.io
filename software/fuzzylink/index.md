...
`dfB` are likely matches for each record in `dfA`. The
[`fuzzylink()`](https://joeornstein.github.io/software/fuzzylink/reference/fuzzylink.md)
...
York in `dfA` and Florida in `dfB`–the
[`fuzzylink()`](https://joeornstein.github.io/software/fuzzylink/reference/fuzzylink.md)
function no longer returns a match for this record; all blocking
variables must match exactly before the function will link two records
together. You can specify as many blocking variables as needed by
inputting their column names as a vector.
...
Because the
[`fuzzylink()`](https://joeornstein.github.io/software/fuzzylink/reference/fuzzylink.md)
function makes several calls to the OpenAI API—which charges a per-token
fee—there is a monetary cost associated with each use. Based on the
package defaults and API pricing as of August 2025, here is a table of
approximate costs for merging datasets of various sizes.
